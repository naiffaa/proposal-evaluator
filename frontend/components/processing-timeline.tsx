...
interface ProcessingTimelineProps {
  stages: string[]
  current: number // index of the in-progress stage
}

export function ProcessingTimeline({ stages, current }: ProcessingTimelineProps) {
  return (
      ...
      {stages.map((stage, i) => {
        ...
        return (
          <li key={stage} className="relative flex gap-4 pb-1">
            ...
            <div className="flex min-h-8 flex-1 items-center py-1">
              <p
                className={cn(
              ...
              {isComplete && (
                <span className="ml-auto text-xs font-medium text-success">Complete</span>
              )}
            </div>
          </li>
        )
